...
/* 	GÖREV 1:
	...
	*/
const br = " ";
function cumlelereDonustur(arr, br) {
  let newArr = [];
  arr.map((item) => {
    newArr.push(item.join(br));
  });
  console.log(newArr);
  return newArr;
}
cumlelereDonustur(cumleler, br);
...
function paragrafOlustur(cumleler, cumleKur, cumlelereDonustur) {
  let newdizi = cumlelereDonustur(cumleler, " ");
  let newParag = cumleKur(
    newdizi[1] + newdizi[3] + newdizi[5] + newdizi[7] + newdizi[9]
  );
...
console.log(
  emojileriDonustur(
    "yani hem :d hem de :D sembolleri 😁'a dönüşmelidir",
    emojiler
  )
);
